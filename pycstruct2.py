...
class ObjectFrame:
    def __init__(self, body_def: str, parents: list) -> None:
        ...
        self.instance_name: str = body_def[
            body_def.rfind("}") + 1 : body_def.rfind(";")
        ].strip()  # struct S {} s;
    ...
    def reg_macros(self, base_name) -> list[str]:
        ...
        for v in self.vars:
            ...
            var_name: str = (
                f'{self.combo_name + ("." if self.combo_name else "")}{v.name}'
            )
            print(var_name)
            ...
            if v.size == 0:
                macros.append(
                    f'REGISTER_VAR({base_name}, {var_name}, {v.ctype}, "{printf}", {stonum});'
                )
            elif v.ctype == "char":
                macros.append(f"REGISTER_CHAR_ARR({base_name}, {var_name})")
            else:
                macros.append(
                    f'REGISTER_ARR({base_name}, {var_name}, {v.size}, {v.ctype}, "{printf}", {stonum});'
                )
        ...
        for b in self.bitfields:
            ...
            for var_name, _ in b.fields:
                var_name = (
                    f'{self.combo_name+ ("." if self.combo_name else "")}{var_name}'
                )
                print(var_name)
                macros.append(
                    f'REGISTER_BITFIELD({base_name}, {var_name}, {b.ctype}, "{printf}", {stonum});'
                )
...
def main(make_includes: list[str]) -> None:
    ...
    for registered_type, registered_name, pragma_pack in registered_structs:
        ...
        print(
            f'Pycstruct: Generating macros for: {registered_type} "{registered_name}"'
        )
        ...
        registers += (
            f"    REGISTER_INTERNAL_STRUCT(_{registered_type}, {registered_name});\n"
        )
        registers += (
            "    "
            + "\n    ".join(
                defined_structs[registered_type].reg_macros(registered_name)
            )
            + "\n\n"
        )
    ...
    struct_cpp = re.sub(
        r"//\s*pycstruct_shit.*", "//pycstruct_shit\n", struct_cpp, flags=re.DOTALL
    )
