import json
from .components import RFComponent, Amp, Loss, Mixer
from .util import return_loss_to_VSWR
...
def parse_amp(name: str, spec: dict[str, float]) -> RFComponent:
...
def parse_mixer(name: str, spec: dict[str, float]) -> RFComponent:
...
def parse_lossy(name: str, spec: dict[str, float]) -> RFComponent:
    ...
    for category, parts_list in catalog.items():
        if category == "Amps":
            for name, spec in parts_list.items():
                components[name] = parse_amp(name, spec)
        elif category == "Mixers":
            for name, spec in parts_list.items():
                components[name] = parse_mixer(name, spec)
        else:  # pretend everything else is default lossy component
            for name, spec in parts_list.items():
                components[name] = parse_lossy(name, spec)
    return components
