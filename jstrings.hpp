...
inline bool contains(std::string str, std::string key)
{
    return str.find(key) != std::string::npos;
}

inline bool contains_any(std::string str, std::string chars)
{
    for(char c : chars)
    {
...
inline bool contains_all(std::string str, std::string chars)
{
    for(char c : chars)
    {
        if(str.find(c) == std::string::npos)
...
inline void left_pad(std::string* str, int new_size, char pad_char = ' ')
{
    assert(str != nullptr);
...
inline std::string left_pad(std::string str, int new_size, char pad_char = ' ')
{
    JStrings::left_pad(&str, new_size, pad_char);
    return str;
}

inline void right_pad(std::string* str, int new_size, char pad_char = ' ')
{
    assert(str != nullptr);
...
inline void replace(std::string* str, std::string key, std::string sub, ReplaceBehavior rb = All)
{
    assert(str != nullptr);
...
inline std::string
replace(std::string str, std::string key, std::string sub, ReplaceBehavior rb = All)
{
...
inline void remove(std::string* str, std::string key, ReplaceBehavior rb = All)
{
    assert(str != nullptr);
    JStrings::replace(str, key, "", rb);
}
inline std::string remove(std::string str, std::string key, ReplaceBehavior rb = All)
{
...
inline JStringList split(std::string str, std::string key, SplitBehavior sb = None)
{
    ...
    if(sb & SkipEmpty)
    {
        strs.erase(std::remove_if(
            strs.begin(), strs.end(), [](const std::string& s) { return s.length() == 0; }));
    }
...
inline std::string
join(const JStringList& strs, std::string sep, std::string head = "", std::string tail = "")
{
    if(strs.size() == 0)
        return "";
...
inline std::string as_bin(Number_t val)
{
    ...
    char buf[buf_size];
    buf[buf_size - 1] = '\0';
    for(int iBit = 0, iBuf = buf_size - 2; iBit < sizeof(val) * 8 && iBuf >= 0; iBit++, iBuf--)
    {
        if(iBit > 0 && (iBit % 8 == 0))
        {
            buf[iBuf] = '_';
